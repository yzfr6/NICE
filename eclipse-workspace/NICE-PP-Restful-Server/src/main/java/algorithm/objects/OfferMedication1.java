...
public class OfferMedication1 {

	public JSONObject medication_request;
	public int id;
	
	public OfferMedication1(int id, JSONObject medication_request) {
		//super("algorithm_offer_medication_1");
		this.medication_request = medication_request;
		this.id = id;
	}
	
	public OfferMedication1(Connection c, int entity_id) throws SQLException, ParseException {
		//super("algorithm_offer_medication_1");
		//this.algorithm_entity_id = entity_id;
		PreparedStatement ps1 = c.prepareStatement("select * from algorithm_offer_medication_1 acm1 inner join drug d on d.id = acm1.drug_id where acm1.algorithm_entity_id = ?");
